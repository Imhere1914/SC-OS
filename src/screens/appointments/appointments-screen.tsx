...
function ApptDialog({ open, initial, title, contacts, onClose, onSubmit, isSubmitting }: {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 p-4 sm:items-center" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input value={form.location} onChange={(e) => set('location', e.target.value)} placeholder="Office, video link, phone…"
              className="w-full rounded-lg border border-[var(--theme-border)] bg-[var(--theme-input)] px-3 py-1.5 text-xs text-[var(--theme-text)]" />
          </div>
          <div>
  ...
  return (
    <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)]">
      ...
      <div className="flex items-center justify-between border-b border-[var(--theme-border)] px-4 py-3">
        <button onClick={prev} className="rounded-lg p-1.5 hover:bg-[var(--theme-hover)] text-[var(--theme-muted)]">‹</button>
        <h2 className="text-sm font-semibold text-[var(--theme-text)]">{MONTH_NAMES[month]} {year}</h2>
        <button onClick={next} className="rounded-lg p-1.5 hover:bg-[var(--theme-hover)] text-[var(--theme-muted)]">›</button>
      </div>
      {/* Day headers */}
      <div className="grid grid-cols-7 border-b border-[var(--theme-border)]">
        {DAY_HEADERS.map(d => (
      ...
      <div className="grid grid-cols-7">
        {cells.map((day, i) => {
          if (!day) return <div key={`blank-${i}`} className="border-b border-r border-[var(--theme-border)] min-h-[80px]" />
          ...
          return (
            <div
              ...
              className={cn(
                'relative cursor-pointer border-b border-r border-[var(--theme-border)] min-h-[80px] p-1.5 transition-colors hover:bg-[var(--theme-hover)]',
                (i + 1) % 7 === 0 && 'border-r-0',
              )}
            >
              <div className="flex items-start justify-between">
                <span
                  className={cn('flex h-5 w-5 items-center justify-center rounded-full text-[11px] font-medium',
                    isToday ? 'text-white font-bold' : 'text-[var(--theme-muted)]')}
              ...
              <div className="mt-1 space-y-0.5">
                {dayAppts.slice(0, 3).map(a => (
                  <button
                    ...
                    className="w-full truncate rounded px-1 py-0.5 text-left text-[9px] font-medium text-white transition-opacity hover:opacity-80"
                    style={{ background: STATUS_COLORS[a.status] }}
                  >
                    {new Date(a.starts_at).toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' })} {a.title}
                  </button>
                ))}
                {dayAppts.length > 3 && (
                  <p className="pl-1 text-[8px] text-[var(--theme-muted)]">+{dayAppts.length - 3} more</p>
                )}
              </div>
            </div>
          )
        ...
        <header className="rounded-2xl border border-primary-200 bg-primary-50/85 p-4 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-base font-semibold text-[var(--theme-text)]">Appointments</h1>
              {apptsQuery.data && <span className="ml-1 text-xs text-[var(--theme-muted)]">({apptsQuery.data.length})</span>}
            </div>
            <div className="flex items-center gap-2">
              <button onClick={invalidate} className="rounded-lg p-1.5 transition-colors hover:bg-[var(--theme-hover)]" title="Refresh">
              ...
              <div className="flex rounded-lg border border-[var(--theme-border)] bg-[var(--theme-card)] p-0.5">
                ...
                <button onClick={() => setViewMode('calendar')}
                  className={cn('rounded-md px-2 py-1 transition-colors', viewMode === 'calendar' ? 'bg-[var(--theme-accent)] text-white' : 'text-[var(--theme-muted)] hover:bg-[var(--theme-hover)]')}>
                  <HugeiconsIcon icon={ViewIcon} size={13} />
                </button>
              </div>
              <button onClick={() => { setCreateDate(null); setShowCreate(true) }}
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {(['upcoming', 'all', 'past'] as const).map((w) => (
                <button key={w} onClick={() => setWhenFilter(w)}
                  className={cn('rounded-full border px-2.5 py-1 text-[11px] font-medium capitalize transition-colors',
                    whenFilter === w ? 'border-transparent text-white' : 'border-[var(--theme-border)] text-[var(--theme-muted)] hover:bg-[var(--theme-hover)]')}
                  style={whenFilter === w ? { background: 'var(--theme-accent)' } : undefined}>
                  {w}
                </button>
              ))}
            </div>
        ...
        {viewMode === 'list' && (
          <div className="flex-1 space-y-4">
            {apptsQuery.isLoading ? (
              <div className="flex items-center justify-center py-12 text-sm text-[var(--theme-muted)]">Loading…</div>
            ) : listAppts.length === 0 ? (
              ...
              grouped.map((group) => (
                <div key={group.label}>
                  <h2 className="mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-[var(--theme-muted)]">{group.label}</h2>
                    ...
                    <AnimatePresence mode="popLayout">
                      {group.items.map((a) => (
                        <motion.div key={a.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                          ...
                          <div className="mt-0.5 h-10 w-1 shrink-0 rounded-full" style={{ background: STATUS_COLORS[a.status] }} />
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <h3 className="truncate text-sm font-medium text-[var(--theme-text)]">{a.title}</h3>
                              <span className="rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase"
                                style={{ background: 'var(--theme-bg)', color: STATUS_COLORS[a.status] }}>
